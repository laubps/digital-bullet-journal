...
import { listMoodHistory } from '@/lib/mood/history';
import { MOODS } from '@/lib/mood/validation';
...
export async function GET(request: Request) {
  ...
  if (mood && !(MOODS as readonly string[]).includes(mood)) {
    return NextResponse.json({ error: 'unknown mood', field: 'mood' }, { status: 400 });
  }

  try {
    const entries = await listMoodHistory(session.userId, {
      from,
      to,
      categoryId,
      mood,
    ...
    console.error('[api/mood/history] error:', err);
